...
Function Convert-FromUnixDate ($UnixDate) {
    # 方法1: 失败
    # GitHub Actions: The term 'Set-TimeZone' is not recognized as a name of a cmdlet
    # Set-TimeZone -Id "China Standard Time"
    # 失败: 发现尽管设置了时区, 但下方 CurrentTimeZone 还是没变, 不受影响
    # [timezone]::CurrentTimeZone.ToLocalTime(([datetime]'1/1/1970').AddSeconds($UnixDate))

    # 方法2
    $cstzone = [System.TimeZoneInfo]::FindSystemTimeZoneById("China Standard Time")
    $csttime = [System.TimeZoneInfo]::ConvertTimeFromUtc(([datetime]'1/1/1970').AddSeconds($UnixDate), $cstzone)
    return $csttime
}
